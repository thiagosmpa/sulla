...
const Usuarios: React.FC = () => {
  ...
  const [qrCode, setQrCode] = useState<string | null>(null); // Adicione este estado
  ...
  const handleUpdate = async () => {
    if (editUser) {
      ...
      try {
        ...
        setUsuarios((prev) =>
          prev.map((user) =>
            user.sessionId === updatedUser.sessionId ? updatedUser : user
          )
        );
        setEditUser(null);
      } catch (error) {
  ...
  const handleRemove = async (sessionId: string) => {
    ...
    try {
      await axios.post(DELETE_URL, { sessionId });
      setUsuarios((prev) =>
  ...
  const handleConnect = async (sessionId: string) => {
    try {
      const response = await axios.post(CONECTAR_URL, { sessionId: sessionId });
      if (response.data.qr) {
      ...
      alert(`User ${sessionId} - ${error.response?.data?.error || error.message}`);
    }
  };

  if (loading) {
    return (
      <div className="w-full h-full flex flex-col justify-center items-center text-center bg-black text-white">
          ...
          <table className="min-w-full table-auto bg-gray-800 rounded-lg">
            ...
            <tbody>
              {usuarios.map((usuario) => (
                <tr
                  key={usuario.sessionId}
                  className="border-t border-gray-700"
                >
                  <td className="py-3 px-4">{usuario.sessionId}</td>
                  ...
                  <td className="py-3 px-4 flex space-x-4">
                    ...
                    >
                      {loadingUpdate && editUser?.sessionId === usuario.sessionId
                        ? "Atualizando..."
                        : "Editar"}
                    </button>
                    <button
                      onClick={() => handleRemove(usuario.sessionId)}
                      className="text-red-500 hover:underline"
                    >
                      {loadingDelete ? "Removendo..." : "Remover"}
                    </button>
                    <button
                      onClick={() => handleConnect(usuario.sessionId)}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center">
            <div className="bg-gray-800 p-4 rounded shadow-lg max-w-md w-full h-1/2">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditUser({ ...editUser, agenda: e.target.value })
                }
                className="mb-2 p-2 w-full text-black"
              />
              <label
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEditUser({ ...editUser, instructions: e.target.value })
                }
